...
def test_with_two_columns(node1, search_no_compare):
    node2 = node1.add_meter(200, search_no_compare.bbox_size + 50)
...
def test_with_two(node1, search_no_compare):
    node2 = node1.add_meter(search_no_compare.bbox_size + 50, search_no_compare.bbox_size + 50)
    manager = Manager(bbox=Bbox.from_leftdown_rightup(node1, node2), job_queue_name='dummy')
    columns = manager._calc_columns()
    rows = manager._calc_rows()
    assert rows == 2
...
def test_with_three(node1, search_no_compare):
    node2 = node1.add_meter(2 * search_no_compare.bbox_size + 50, 2 * search_no_compare.bbox_size + 50)
    manager = Manager(bbox=Bbox.from_leftdown_rightup(node1, node2), job_queue_name='dummy')
    columns = manager._calc_columns()
    rows = manager._calc_rows()
    assert rows == 3
